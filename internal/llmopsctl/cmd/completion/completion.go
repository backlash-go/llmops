...
var (
	completionLong = templates.LongDesc(`
		...
		completion of llmopsctl commands.  This can be done by sourcing it from
		the .bash_profile.
	...
	completionExample = templates.Examples(`
		...
		## If llmopsctl is installed via homebrew, this should start working immediately.
		## If you've installed via other means, you may need add the completion to your completion directory
		    llmopsctl completion bash > $(brew --prefix)/etc/bash_completion.d/llmopsctl


		# Installing bash completion on Linux
		## If bash-completion is not installed on Linux, please install the 'bash-completion' package
		## via your distribution's package manager.
		## Load the llmopsctl completion code for bash into the current shell
		    source <(llmopsctl completion bash)
		## Write bash completion code to a file and source if from .bash_profile
		    llmopsctl completion bash > ~/.iam/completion.bash.inc
		    printf "
		...
		# Load the llmopsctl completion code for zsh[1] into the current shell
		    source <(llmopsctl completion zsh)
		# Set the llmopsctl completion code for zsh[1] to autoload on startup
		    llmopsctl completion zsh > "${fpath[1]}/_iamctl"`)
)
